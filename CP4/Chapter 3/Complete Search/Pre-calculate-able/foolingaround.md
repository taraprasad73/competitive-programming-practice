...
// Need -lpthread in gcc args, so don't send it to online judge
class CallBackTimer {
public:
 ...
 * 
 * Check if there exists a prime number p, such that when p-1 is subtracted from n,
 * leaves a result which the second player can win. This implies that,
 * when Alice takes out p-1 from n, we are left with a number which second player
 * is known to win. But since Bob is playing now, second player is Alice and she will win.
 * So, in order for Bob to win, no such prime should exist.
 */
set<int> secondPlayerWins;
bool canSecondPlayerWin(int n) {
    if(primes[n + 1] == true)
        return false;
    // known values till now for which second player wins
    for(int x: secondPlayerWins) {
        if(primes[n - x + 1] == true) {
            return false;
        }
    }
    return true;
...
// for MAX_N = 1E9, takes ~13 minutes
void preCalculate() {
